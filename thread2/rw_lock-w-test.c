...
{
	if(rw->writer > 0 || rw->write_counter > 0){
		rw->read_counter++;
...
void w_unlock(struct rw_lock * rw)
{
	...
	if(rw->write_counter != 0 || rw->read_counter != 0){
		if(rw->write_counter >= rw->read_counter)
			pthread_cond_signal(&rw->write_signal);
		else
			pthread_cond_signal(&rw->read_signal);
	}
